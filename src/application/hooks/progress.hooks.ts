...
export const useUserProgress = (): UseUserProgressReturn => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const [progressData, sessionsData, notesData] = await Promise.all([
  ...
  const updateProgress = useCallback(async (
    ...
    try {
      ...
      if (updatedProgress) { await fetchData(); return true; }
      return false;
    } catch (err) {
  ...
  const markAsCompleted = useCallback(async (
    ...
    return updateProgress(contentSlug, contentType, {
      ...
      completed_at: new Date().toISOString(), rating,
  ...
  const bookmarkContent = useCallback(async (
    ...
    return updateProgress(contentSlug, contentType, {
      status: 'bookmarked', bookmark_position: position || 0,
    });
  }, [updateProgress]);
  ...
  const startLearningSession = useCallback(async (
    ...
    try {
      ...
      if (session) { setCurrentSession(session.id); setSessions(prev => [session, ...prev]); return session.id; }
      return null;
    } catch (err) {
  ...
  const createNote = useCallback(async (
    ...
    try {
      ...
      if (note) { setNotes(prev => [note, ...prev]); return true; }
      return false;
    } catch (err) {
  ...
  const updateNote = useCallback(async (noteId: string, updates: Partial<UserNote>): Promise<boolean> => {
    try {
      ...
      if (updatedNote) { setNotes(prev => prev.map(n => n.id === noteId ? updatedNote : n)); return true; }
      return false;
    } catch (err) {
  ...
  const deleteNote = useCallback(async (noteId: string): Promise<boolean> => {
    try {
      ...
      if (success) { setNotes(prev => prev.filter(n => n.id !== noteId)); return true; }
      return false;
    } catch (err) {
