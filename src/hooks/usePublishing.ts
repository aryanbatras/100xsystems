...
export const usePublishing = (articleTitle: string) => {
  ...
  const handlePublish = async (quillRef: any) => {
    ...
    try {
      ...
      log(`📄 Content extracted (${delta.ops?.length || 0} operations)`);
      ...
      log(`📝 Article slug: ${slug}`);
      log(`📝 Slug generated in ${slugTime}ms`);
      ...
      log(`🔍 Scanning content for images... (${imageExtractionTime}ms)`);
      log(`📸 Found ${images.length} image(s) to upload`);
      ...
      if (images.length > 0) {
        ...
        log(`✅ Successfully uploaded ${uploadedImages.length} image(s) in ${imageUploadTime}ms`);
      ...
      log(`📊 Performance: Content ${contentExtractionTime}ms, Images ${imageUploadTime || 0}ms, HTML ${htmlGenTime}ms, GitHub ${publishTime}ms`);
