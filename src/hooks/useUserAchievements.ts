...
import { UserAchievementWithAchievement, Achievement, LearningStreak } from '../services/types/database';
...
export const useUserAchievements = (): UseUserAchievementsReturn => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch achievements data');
    } finally {
      setLoading(false);
    }
  }, [user?.id]);
    ...
    try {
      ...
      if (newlyUnlocked.length > 0) {
        ...
        setLockedAchievements(prev => 
          prev.filter(locked => !newlyUnlocked.some(unlocked => unlocked.achievement_id === locked.id))
        );
